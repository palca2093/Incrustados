...
bool Lab1_utilities::START_UP(unsigned int i_u32Time, short *p_16iBlinkCount, unsigned char i_u8PowerMode)
{
    // Set blinking LEDs Port-Pin
    P2 -> DIR =  i_u8PowerMode;
    P2 -> OUT = 0;
...
bool Lab1_utilities::CHECK_LIGHT(float i_fLightNomlevel, unsigned short i_u16PowerMode)
{

    if(OPT3001_getLux() < i_fLightNomlevel)
    {
        P2 -> OUT = i_u16PowerMode;
        return false;
    }

...
void Scheduler::PairTasks(void)
{
    ...
    for(short l_i16Counter1 = 0; l_i16Counter1 < l_i16ValidTaskSlots; l_i16Counter1++)
    {
        st_TaskInfo l_stTaskH = m_aSchedule[l_i16Counter1];

        for(short l_i16Counter2 = 0; l_i16Counter2 < l_i16ValidTaskSlots; l_i16Counter2++)
           {

            if( (l_stTaskH.pTask -> GetHandledData()) == NULL_DATA )
            {
                break;
            }
                st_TaskInfo l_stTaskN = m_aSchedule[l_i16Counter2];

                if( (l_stTaskH.pTask -> GetHandledData()) == (l_stTaskN.pTask -> GetNeededData()) ){

                    l_stTaskH.pTask -> DestinationID = l_i16Counter2;
                }
